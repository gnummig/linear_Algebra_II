...
$\forall x \in V \Phi(x):= \sum _{j=1}^q {\phi(\alpha_jw_j)} \in \SPAN'(w1,...,wq)\Rightarrow im \Phi \leq \\SPAN'(w1,...,wq)$  (6)\\
sei umgekehrt $w \in  \SPAN'(w1,...,wq)$§ vorgegeben $\Rightarrow \exists \gamma_1,...,\gamma_q \in \mathcal{K}: w=\sum _{j=1}^k {\gamma_jw_j}$\\
$\Rightarrow w= \sum _{j=1}^q \beta_j\Phi(v_i)= \phi (\sum _{j=1}^k\beta_jv_j) \in Im\Phi $\\
$\Rightarrow \SPAN'(w1,...,wq) \subseteqq \IM \Phi $\\
F9.7 seien $V$ und $W \mathcal{K}$- VR wobei $1\leq \dim V<+\inf$, sei $r\in \mathbb{N}$ sowie $v_1,v_2,..,v_r \in V$ und $w_1,w_2,..,w_r \in W$
sei vorrausgesetz dass $v_1,..,v_r$ linear unabhängig, dann gibt es mindestens eine lineare abbildung $\Phi :V\rightarrow W$, mit $\Phi(v_j)=W_j \forall j\in \mathcal{N}_{1,r}$
beweis:  Nach Basisergänzungssatz $r\leq q$ im Fall $r=q$ wende s 9.6 an. im fall $r<q$ können nach Basisergänzungssatz vektoren $v_r+1,...,v_q\in V$ 
derart ergänzt werden, dass $v_1,...,v_r,v_r+1,...,v_q$ eine basis von $V$ ist und wir können S 9.6 anwenden QED
\begin{bemerkung}
 aus dem beweis von folgerung 9.7 ist ersichtlich, dass im fall dass $r<q$ und $W\neq {0_w}$ die lineare abbildung nicht eindeutig bestimmt ist. 
\end{bemerkung}

\begin{lemma}
%L9.8
seien V und W $\mathcal{K}$ VR sowie $\Phi :v\rightarrow W$  eine lineare abbildung, dann gilt $\dim \IM \Phi\leq \dim V$.
%übung Unterscheidung dimV= \inf, dim V\in \n, dim V =0
\end{lemma}
\begin{satz}
S9.9 seien $V$ und $W \mathcal{K}$-Vektorräume  wobei $\dim V < \inf$, sowie $\Phi :V\rightarrow W$  eine lineare abbildung dann gilt $\dim (\IM \Phi\)$+dann gilt \dim (\mathcal{K}er\Phi)= \dim V$
%beweis in Übung
 
\end{satz}

Definition: seien $V$ und $W \mathcal{K}$-Vektorräume  sowie $\Phi :V\rightarrow W$ eine lineare abbildung dann heißt $Rang_K$ von $\Phi := \dim (\IM \Phi)$ der Rang von $\Phi$ 
Satz 9.10 seien $V$ und $W \mathcal{K}$-Vektorräume sowie $\Phi :V\rightarrow W$  eine lineare abbildung
(a) falls $\Phi$ injektiv, so ist die Umkehrbildung $\Phi^{-1} : \IM \Phi \rightarrow V$ ebenfalls linear
(b) im fall dass $\dim V = \dim W <\inf$ gilt, sind folgenda Aussagen äquivalent:
  1 $\Phi$ ist bijektiv
  11 $\Phi$ ist injektiv
